...
//복사생성자
template<class T>
inline DoublySortedLinkedList<T>::DoublySortedLinkedList(const DoublySortedLinkedList<T>& list)
{
	m_nLength = 0;
	m_pFirst = nullptr;
...
//아이템을 검색하는 함수
template<class T>
inline T * DoublySortedLinkedList<T>::SearchItem(const T & item)
{
...
//대입연산자 오버로딩
template<class T>
inline DoublySortedLinkedList<T>& DoublySortedLinkedList<T>::operator=(const DoublySortedLinkedList<T>& list)
{
	m_nLength = 0;
	m_pFirst = nullptr;
	DoublyIterator<T> itor(list);
